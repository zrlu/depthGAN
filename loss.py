...
class HcGANLoss(nn.Module):
    '''
        Half-cycle GAN loss
    '''
    def __init__(self):
        super(HcGANLoss, self).__init__()
    ...
    def forward(self, xs, label):
        if label == True:
            loss_R_real = self.loss(xs, self.real_label.expand_as(xs))
            return loss_R_real
        else:
            loss_R_fake = self.loss(xs, self.fake_label.expand_as(xs))
            return loss_R_fake
...
class MonodepthLoss(nn.modules.Module):
    ...
    def forward(self, depths, left_pyramid, right_pyramid):
        # Prepare disparities
        disp_left_est = [d[:, 0, :, :].unsqueeze(1) for d in depths]
        disp_right_est = [d[:, 1, :, :].unsqueeze(1) for d in depths]
